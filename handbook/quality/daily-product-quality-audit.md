...
历史版本曾修复“无来源扩写”：提示词、摘要校验和结构化解析都拒绝超过候选数或重复 `article_id`。本轮架构调整保留“不能新增来源”的约束，但移除“一个来源只能生成一条新闻”的限制，改由独立 `max_summary_items` 控制总条数；这样聚合日报可以拆出多条有来源的独立事实。

当前契约仍关闭“无输入映射”和 URL 伪造路径；同一输入的重复绑定只有在条目事实不同、URL 一致时才允许，不同条目之间的语义重复仍需要内容质量审查。
...
| 观察到的问题 | 当前行为 | 风险 |
| --- | --- | --- |
| 少量候选也会发布 | 摘要数量校验按输入数量下调，而非独立主题的最小数量 | 4 条候选可形成一次显示成功的正式日报 |
| 内容被凑到 10 条 | 提示词要求在低价值时也选择最接近 AI 或科技主题的 10 条 | 泛科技内容进入 AI 日报 |
| 4 条输入被扩展为 10 条输出 | 摘要结果受独立 `max_summary_items` 上限约束，来源 ID 必须来自输入但允许重复 | 越界或无输入映射结果会阻断发布；同一聚合来源的事实重复仍需内容审查 |
| 同一公司占比过高 | 去重基于规范化标题加域名，不限制实体或主题数量 | 同一公司不同报道可以同时占据多数位置 |
| 一篇新闻被写成多条 | 每条摘要必须绑定已知输入 `article_id` 和同源 URL；同一来源可拆出多条 | 重复改写同一事实仍有内容风险，但不会因合法聚合链接而误阻断 |
...
第一版不需要额外模型。使用标题规范化和小型别名表识别 OpenAI、Anthropic、Google、Meta、Microsoft、Nvidia 等高频主体；每个主体默认最多保留 2 条。所有其他主体已经用尽后才允许回填超过配额的候选。

这会直接把 2026-07-10 的 OpenAI 5/10 降到可控范围，同时保留 Meta、Nvidia、融资和开发工具等已有候选。
