...
本轮已修复：提示词改为“最多 10 条且不超过输入候选数”；摘要校验新增超额拒绝；结构化解析不再接受超出候选数或重复 article_id 的结果；离线摘要也限制为候选数上限，空候选返回“暂无新闻”。新增 4 条候选生成 10 条的回归测试后，摘要会 fail closed，运行阻断发布而不是硬扩展。
...
| 4 条输入被扩展为 10 条输出 | 摘要结果强制候选数上限和唯一 article_id，发布前再次校验 | 越界或无输入映射结果会阻断发布；不同输入之间的语义重复仍需后续主题门禁 |
| 同一公司占比过高 | 去重基于规范化标题加域名，不限制实体或主题数量 | 同一公司不同报道可以同时占据多数位置 |
| 一篇新闻被写成多条 | 每条摘要必须绑定唯一输入 article_id | 同一输入无法绑定多个摘要；跨输入的误绑定仍需主题级检测 |
