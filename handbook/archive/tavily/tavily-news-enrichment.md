...
> 统一入口：当前 Tavily 接入状态、进度、风险和下一步已经合并到 `handbook/operations/tavily.md`。本文保留为历史方案与实验记录，不再作为当前状态的唯一依据。
...
- 分层 trusted domains 草案：`handbook/archive/tavily/tavily-trusted-domains-draft.md`
...
历史回放时，`within_24h` 不是按当前日期硬算，而是按样本所属日报日期加项目文档里的调度时间 `08:36 Asia/Shanghai` 作为参考点计算。
