...
`2026-06-17` Tavily Gray Daily artifact。

## 一句话结论

Tavily 已经完成 Phase 0 研究、trusted domains 实验、replay harness、正式模块第一版、`main.py` 接线和独立 GitHub Actions 灰度 workflow；但它仍是默认关闭的能力，没有进入生产 deploy workflow，也还没有达到可长期默认开启的稳定度。
...
| GitHub Actions 隔离灰度 | `Tavily Gray Daily` 已可用，生产 deploy 灰度入口已关闭 | 70% |
| 默认开启 | 不建议开启 | 0% |

当前决策：继续保持 `enrichment.enabled: false`。本地可用 `--enrichment on` 做开发验证；GitHub 上只通过 `Tavily Gray Daily` 做隔离灰度。不要把 Tavily 改成常驻 source，也不要因为单日结果差就扩大白名单。
...
- GitHub Actions 只有独立 `Tavily Gray Daily` 注入 `TAVILY_API_KEY`。
- `Daily Report Deploy` 不注入 `TAVILY_API_KEY`，不提供 `enable_tavily`，定时任务仍跟随默认关闭配置。
...
当前 GitHub Actions 只保留一个 Tavily 灰度入口：`.github/workflows/tavily-gray.yml`，显示名称为 `Tavily Gray Daily`。

当前边界：

- `Daily Report Deploy` 不注入 `TAVILY_API_KEY`，不追加 `--enrichment on`。
- `Tavily Gray Daily` 使用 `secrets.TAVILY_API_KEY`，运行 `python3 main.py run --offline --enrichment on`。
- `Tavily Gray Daily` 不提交、不发布、不部署，只上传 `gray/tavily/YYYY-MM-DD/` artifact。
- 缺少 `TAVILY_API_KEY` 时，`Tavily Gray Daily` 直接失败，避免把无 key 样本误判为策略质量。

因此，生产每日任务仍不会验证 Tavily；所有 GitHub Actions Tavily 观察都从 `Tavily Gray Daily` 的 `scorecard.json`、`scorecard.md` 和 `enrichment-summary.json` 复盘。
...
- source 为 0 条时的诊断说明。
- 独立 `Tavily Gray Daily` workflow 和 `.env.example` key 示例。
...
- GitHub Actions 已有独立灰度 runner 样本，但还缺多日稳定样本。
- 还没有多日稳定样本证明默认路径可靠。
...
### P1: Tavily Gray Daily 稳定性样本不足

当前 Actions 已提供独立 `Tavily Gray Daily`，并在 2026-06-17 手动 run 中验证 key 可用、请求成功、artifact 可复盘。

计划：

- 保持 `Daily Report Deploy` 不启用 Tavily。
- 继续观察 `Tavily Gray Daily` 的 scheduled/manual run。
- 每次验证后检查 artifact 中的 `scorecard.json`、`scorecard.md` 和 `enrichment-summary.json`。
- 至少收集多日无 `http_error` 样本后，再评估预算、domain 或 query 单变量实验。
...
### 下一步 1: 继续观察 Tavily Gray Daily

只验证隔离灰度，不改每日默认。

验收：

- `Daily Report Deploy` 不追加 `--enrichment on`。
- `Tavily Gray Daily` 能读取 `TAVILY_API_KEY` 并完成 API 请求。
- artifact 中的 `scorecard` 能复盘 Tavily 是否执行、失败在哪个阶段、保留了哪些原始文章。
