...
本文状态基准：`2026-06-23`，基于当前工作区代码、文档和
`2026-05-11` 以来的 Tavily GitHub Actions 灰度 artifact。

## 一句话结论

Tavily 已从 “AI-only strict verifier” 调整为 “technology news additive enrichment”：source 有效文章默认保留，verify 负责标注置信度，refill 负责补入可信科技新闻。`config.yaml` 中仍默认关闭，生产发布 workflow 的定时任务仍不默认启用 Tavily，也还没有达到可长期默认开启的稳定度。
...
| 正式模块接入 | 已切换为 source-preserve + tech-news refill | 75% |
| 诊断与 fail-open | 已有 source preserved / strict refill / soft refill / safety gate 诊断 | 75% |
| 生产 GitHub Actions 接线 | 已有手动 `enable_tavily` 开关、独立 `tavily-gray` 定时灰度与 safety gate 回写路径，仍需多日样本验证 | 70% |
| 默认开启 | 不建议开启 | 0% |

当前决策：继续保持 `enrichment.enabled: false`，用 `--enrichment on` 做本地或手动灰度验证。不要把 Tavily 改成常驻 source，也不要因为单日结果差就扩大白名单；先看 `final_count_delta_vs_source` 和 `added_by_tavily_count`。
...
- 本地 prefilter，按 `ai_core`、`tech_core`、`tech_business`、`tech_adjacent`、`generic_or_low_signal`、`hard_reject` 分层。
- exact verify，并用 `verification_status` 标注 `verified`、`matched_but_stale`、`matched_missing_date`、`no_match_preserved`、`request_error_preserved`。
- source-preserve：缺 Tavily 匹配、缺日期或请求失败时默认不删除有效 source 文章。
- source preserved 后不足目标时触发 priority refill。
...
- refill 合并阶段 near-duplicate / story-cluster 拦截，按 topic bucket allowlist 接受可信科技新闻。
- `strict_refill` 与 `soft_refill` 分开计数；soft-date 只在 source 不足且候选处于宽松窗口内时使用。
- request timeout / HTTP error / connection error 分类。
- `source_preserved_count`、`hard_rejected_candidates`、`preserved_unverified_candidates`、`added_by_tavily_count`、`final_count_delta_vs_source` 等诊断字段。
- `accepted_by_stage_preview` 区分 `source_preserved`、`strict_refill`、`soft_refill`。
...
- `boundary_mode: tech_news`: 科技新闻优先，AI 是高优先级子集。
- `preserve_source_on_verify_failure: true`: source 有效文章不因 Tavily no-match / missing date / outside strict window 被删除。
- `min_articles: 10`: 目标新闻数，但不强行凑数。
- `max_articles_after_enrichment: 14`: Tavily 最多补到该上限；不会为了上限裁掉有效 source。
- `strict_hours: 24`: 严格时间窗。
- `max_total_calls: 10`: 单次运行 Tavily 总预算。
- `max_verify_calls: 4`: exact verify 上限；实际 verify budget 会先从
  `max_total_calls` 扣除 priority + secondary refill 预留调用。
- `max_refill_rounds: 2`: 每个 refill stage 默认最多 2 轮。
- `refill_queries`: 多 query 科技新闻补量，不再只搜 AI 公司和模型发布。
- `verify_search_depth: basic`: verify 用低成本路径。
- `refill_search_depth: advanced`: refill 用更深搜索。
...
| `source_preserved_count` | hard reject 后保留的有效 source 文章数 |
| `source_dropped_count` | hard reject 导致的 source 丢弃数 |
| `hard_rejected_count` | 缺标题/链接、聚合垃圾或明确非科技营销内容的 hard reject 数 |
| `preserved_unverified_count` | 未被 Tavily 严格验证但仍保留的 source 数 |
| `added_by_tavily_count` | Tavily 相对 source preserved 净增文章数 |
| `final_count_delta_vs_source` | `final_count - source_preserved_count`，灰度回写红线要求不小于 0 |
| `prefiltered_count` | 进入 verify 的本地候选数 |
| `prefilter_stats` | 本地预筛统计 |
| `topic_bucket_counts` | topic bucket 分布 |
...
| `strict_refill_accepted_count` | strict 时间窗内接受的 refill 数 |
| `soft_refill_accepted_count` | soft-date 宽松窗口内接受的 refill 数 |
...
| `preserved_unverified_candidates` | verify 未通过但保留的 source 明细 |
| `hard_rejected_candidates` | 真正从 source 中删除的候选明细 |
...
- verify no-match、outside strict window、missing published date 时保留 source 并记录 `verification_status`。
- source preserved 已达到 `min_articles` 时不再触发 refill。
- priority refill 不足时继续调用 secondary refill，并只接收剩余缺口需要的候选。
- 默认预算会保留 secondary refill 机会，避免 verify 用满 `max_total_calls`。
- prefilter 分层为 `ai_core`、`tech_core`、`tech_business`、`tech_adjacent`、`generic_or_low_signal`、`hard_reject`，并按该顺序消耗 verify 预算。
- 聚合型标题在 verify 前硬拒绝。
- refill 保持科技相关性门禁，可信非 AI 科技新闻可以进入 `strict_refill` 或 `soft_refill`。
- scorecard 输出 `safety_gate`，当 `final_count < source_valid_count` 时阻止灰度回写。
...
- 断言旧灰度事实仍可作为回归背景，但当前 source-preserve 会保留有效 source。
- 断言当前默认预算：`reserved_refill_calls=2`，`verify_budget=5`。
- 断言 source preserved 后仍有目标缺口时 priority refill 可以净增文章。
