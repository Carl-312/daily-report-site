...
- 公开摘要接口已移除伪 `stream` 参数；交付模式由 endpoint/model capability 私下选择
  `non_stream` 或完整聚合后再校验的 `buffered_stream`。
...
- `buffered_stream` collector 已实现 reasoning/content 隔离、结束信号校验、usage/response hash
  聚合，并在 Kimi K2.7 ModelScope 精确路由上完成 live 验证；未经同等验证的 capability 仍不得启用。
...
1. 公开摘要接口只表达“返回完整、已校验结果”，移除没有生效的 `stream` 参数和旧
   `_summarize_stream()`。网络交付由精确 capability 决定；流式内容只在内存聚合，绝不边生成边发布。
...
模型执行策略
  └─ 输出 token、单次超时、provider 总预算、最大尝试次数、可重试错误
       ↓
Completion 收集器
  └─ non_stream；已验证 capability 可选 buffered_stream
       ↓
统一解析与发布门禁
...
### 第二阶段：仅在有证据时增加 `buffered_stream`（已按精确 capability 实施）

Kimi K2.7 的 ModelScope live probe 已证明其标准 SSE 可完整聚合，因此已新增 provider-neutral
buffered stream collector。它满足：
...
- 已在 live probe 证明 Kimi K2.7 的 SSE envelope、结束信号和 reasoning/content 可完整聚合后实施。
- Kimi capability 进入显式 smoke/shadow，不替换当前默认非流式生产路径。
- 其他 endpoint/model 不继承该结论，未经独立证据继续使用 `non_stream` 或保持禁用。
...
13. `buffered_stream` 只聚合最终 `delta.content`，reasoning 只计长度；缺少终止信号、reasoning-only
    或截断响应均不可发布。
...
- 将目标 capability 的 `delivery_mode` 设为 `non_stream`，即可停用 buffered stream collector；若该
  provider 不支持非流式单文档响应，应直接移出候选链，而不是强行降级协议。
