...
**2026-07-14 Shadow 第 1 天证据：** 非发布 GitHub 运行在 `enable_agihunt=true`、
`enable_tavily=false`、`publish=false` 下成功完成。preview
...
`Tencent-Hunyuan/Hy3` 并完成第二次非发布 GitHub 运行：AGIHunt health gate 通过，
但主 ModelScope 与 Hunyuan 尝试均因空摘要触发
`SummaryQualityError`，最终回退到 SiliconFlow。因此 Hunyuan 尚未被账户验证，本次同日
模型试验不增加 7 天 shadow 的通过日。上述早期运行记录已于 2026-07-21 按灰度清理删除。
