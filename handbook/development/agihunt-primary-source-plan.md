...
ModelScope endpoint/token 以“无可用 provider”拒绝。维护者已要求把第二候选切换为
`Tencent-Hunyuan/Hy3`，并以一次非发布 GitHub 灰度验证；该结果出来前，不能宣称新模型
已经被账户授权或满足模型要求。
...
6. 当前 ModelScope endpoint/token 尚未启用 Kimi K2.7 Code provider；第二候选已改为
   `Tencent-Hunyuan/Hy3`，仍需一次真实 GitHub 灰度验证后才能标记为已验收。
