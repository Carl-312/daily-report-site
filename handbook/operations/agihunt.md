...
- 当次摘要没有满足 ModelScope Kimi 要求：当前 endpoint/token 对 Kimi K2.7 Code 返回
  ...
  `Tencent-Hunyuan/Hy3`，待一次非发布 GitHub 灰度验证；失败时仍安全回退到 SiliconFlow。
...
现有摘要器已经把 ModelScope `Tencent-Hunyuan/Hy3` 放在主模型之后的第二候选位置。它只在需要生成日报摘要时参与模型回退，不参与 AGIHunt 抓取、筛选或事实扩展。
