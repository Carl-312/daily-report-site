...
本文状态基准：`2026-07-23`。2026-05 至 2026-07-19 的 benchmark、refill 和早期灰度数据仅作为
历史证据保留；当前生产策略以紧接着的小节为准。

## 2026-07-23 当前生产策略

本节描述当前代码。真实 GitHub Actions 灰度产物完成检查前，只能宣称本地实现和测试已完成：

- 抓取元数据确定候选队列，Tavily 不得从队列外加入独立新闻；移除与此冲突的 bounded refill。
- 候选队列确定性排序并在前段做简单主体轮转，不增加 AI 预选。
- 每日 Tavily 请求队列最多 30 条，超出部分不再请求 Tavily；其中直接 Story 保留 source
  adapter 的原始证据并继续参与选题，未补证 Lead 只写入私有诊断。
- 队列中的 Lead 和直接 Story 统一执行最多两轮 Tavily。Lead 必须补证成功；直接 Story 补充
  失败时保留原证据。
- 通用证据门禁只检查同一主要主体、同一事件身份、直接文章 URL、真实发布时间和足够正文；
  排除已知非证据页面，但不使用封闭可信域名 allowlist 或复杂来源信誉评分。
- 单个合格直接来源即可发布，多来源印证只提高置信度与信息密度。
- 每日 Tavily 调用硬上限为 30 次；最终已有 10 条合格 Story 时也不提前停止，直到 30 次、候选
  耗尽或软截止。不得为了用满预算搜索元数据队列外新闻。
- HTTP 432/433 统一记录为 `usage_limit_exceeded` 并作为 terminal outcome，只消耗当前一次调用，
  不再把账户额度耗尽误记为普通服务端错误并继续请求。
- 每条候选保存原始元数据与最多三份结构化 Tavily 证据，不增加中间研究 AI；最终摘要 AI 只调用
  一次。
- 成稿事实句严格受证据约束，意义句允许有限编辑推断；最终主新闻的每日目标和发布上限均为 10 条，证据不足时允许更少。
- 0 条是正常可发布的当天空日报；当前自动移除最近三天已入选的完全相同 URL，语义相近但 URL
  不同的跨日事件仍由维护者复核。
- 英文原标题、逐条出处、observation signal 和运行诊断保留在结构化数据或日志，不进入读者正文。

完整产品边界与目标样本见
[`../quality/2026-07-19-content-value-analysis.md`](../quality/2026-07-19-content-value-analysis.md#已确认的产品边界2026-07-21)。

## 2026-07-19 历史生产策略（已被上节替代）

```text
direct story ──────────────────────────────┐
title/trending lead → 2-round resolution ──┤
                                            ▼
                                  publishability gate
                                            ▼
                           verify / bounded refill / summary
```
...
- `agihunt_trending` 与 TechCrunch title-only 只负责发现，解析失败后进入
  “观察信号（未证实）”，不进入主新闻。
...
Tavily 已经完成 Phase 0 研究、trusted domains 实验、replay harness、正式模块第一版、`main.py` 接线和 GitHub Actions 手动灰度入口；但它仍是默认关闭的灰度能力，还没有进入 GitHub Actions 定时默认路径，也还没有达到可长期默认开启的稳定度。
...
| 生产 GitHub Actions 接线 | 已有手动 `enable_tavily` 开关与 key 注入路径，待 runner 实跑验证 | 55% |
| 默认开启 | 不建议开启 | 0% |

当时决策是保持默认关闭；该决策已被上方 2026-07-19 生产策略替代。
...
- GitHub Actions 只在手动触发且 `enable_tavily=true` 时注入 `TAVILY_API_KEY`。
- 定时任务不显式启用 Tavily，仍跟随默认关闭配置。
...
## 2026-07-13 历史 GitHub Actions 状态

当前 `.github/workflows/deploy.yml` 已提供 Tavily 手动灰度入口，但不会在定时任务中默认开启 Tavily。
...
- `TAVILY_API_KEY` 只在手动触发且 `enable_tavily=true` 时注入。
- `enable_tavily=false` 和 schedule 定时任务不会追加 `--enrichment on`。
- 手动启用但缺少 `TAVILY_API_KEY` 时，主流程应完成并由 enrichment 层安全降级。
- 尚未归档或突出展示 `data/YYYY-MM-DD.json` 中的 enrichment 诊断。
