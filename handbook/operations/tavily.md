...
本文状态基准：`2026-07-21`。2026-05 至 2026-07-19 的 benchmark、refill 和早期灰度数据仅作为
历史证据保留；当前生产策略以紧接着的 2026-07-21 小节为准。
...
- 每日正式候选队列最多 30 条，超出部分由同一元数据顺序截断并写入私有诊断，确保入队候选至少
  经过一轮 Tavily。
...
- 0 条是正常可发布的当天空日报；跨日重复不新增自动语义判断，由维护者人工复核。
